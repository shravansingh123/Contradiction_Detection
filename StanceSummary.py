...
topnig="topnIG.bin"
rtstatus=0 #1 states that retweet will be considered 0 otherwise
def finalprobscore(categorydir):
    tweetsA,tweetsB=TwtstanceIndication1(categorydir) #uncomment later    
    n=30
    #artscoreA=ArticulationScore(tweetsA,categorydir)    
    #artscoreB=ArticulationScore(tweetsB,categorydir)    
    summaryA=[x+"\n" for x in tweetsA]
    summaryB=[x+"\n" for x in tweetsB]
    with open("Outputmag1.txt","a") as f:
        f.writelines("\ntop %s, stances A based tweets: %s\n" % (n/2,categorydir))
        f.writelines(summaryA) 
        f.writelines("\ntop %s, stances B based tweets: %s\n" % (n/2,categorydir))
        f.writelines(summaryB) 
    '''print("top A stances based tweets:\n")    
    for x in summaryA:
        print x
    print("top B stances based tweets:\n")    
    for x in tweetsB:
        print x    '''
        
def finalprobscore0(categorydir):
    tweets,stanceA,stanceB,Ha,Hb=TwtstanceIndication(categorydir) #uncomment later    
    n=30
    sortedA=sorted(stanceA,key=lambda x:x[1], reverse=True)#[0:n/2]
    sortedB=sorted(stanceB,key=lambda x:x[1], reverse=True)#[0:n/2]         
    print sortedA,sortedB
    summaryA=[tweets[x[0]][0]+"\n" for x in sortedA]
    summaryB=[tweets[x[0]][0]+"\n" for x in sortedB]
    with open("Outputmag1.txt","a") as f:
        f.writelines("\ntop %s, stances A based tweets: %s\n" % (n/2,categorydir))
        f.writelines(summaryA) 
        f.writelines("\ntop %s, stances B based tweets: %s\n" % (n/2,categorydir))
    ...
    for x in summaryB:
        print x    
    '''    
    #print(len(stanceA),len(stanceB))
    with open("tweets.bin","wb") as f:
        pickle.dump(tweets,f)
    with open("tweets.bin","rb") as f:
        tweets=pickle.load(f)    
    artscore=ArticulationScore(tweets)
    
    index=sorted(range(len(artscore)),key=lambda x:artscore[x],reverse=True)[0:n]
    print("top 5 articulate tweets:\n")
    for x in index:
        print (tweets[x][0],artscore[x])
...
def ArticulationScore(tweets):
    ...
    dftrain=xvalsetter("non_articulate_tweets.txt",badwords,oov)        #training data
...
def xvalsetter(filename,badwords,oov):
    ...
    with open("POStagtwt.txt","r") as f:
...
def TwtstanceIndication1(categorydir):
    nodes=GraphGeneration1(categorydir)    
    with open(categorydir+"/"+nodedict, 'wb') as f:
        pickle.dump(nodes,f)
    with open(categorydir+"/"+nodedict, 'r') as f:
        nodes = pickle.load(f)
    print nodes
    with open(categorydir+"/graphfile.txt.part.2","r") as file:    #partitioned file having number 1 and 0 for 2 partitioning
    ...
    nodeA=[key for key,value in nodes.iteritems() if color[value-1]==0]
    nodeB=[key for key,value in nodes.iteritems() if color[value-1]==1]
    print "node node B:"
    print nodeB
    tweetsA=fetchtweets(categorydir,nodeA)#tweets structure tuple(user,txt)        
    tweetsB=fetchtweets(categorydir,nodeB)#tweets structure tuple(user,txt)        
    print tweetsB
    return [x[1] for x in tweetsA],[x[1] for x in tweetsB]
...
def TwtstanceIndication(categorydir):
    ...
    with open(categorydir+"/graphfile.txt.part.2","r") as file:    #partitioned file having number 1 and 0 for 2 partitioning
        color=[int(i) for i in file.readlines()]        
    #currently tweets that are authored by community member is only included(same ht could have been by other user also);    
    tweets=fetchtweets(categorydir,nodes.keys())#tweets structure tuple(user,txt)    
    ht=fetchhashtags(categorydir,tweets,nodes,color)    
    HsStanceIndication.state=0
    pshAscore=HsStanceIndication(ht,tweets,nodes,color,0)#returns normalised IG of Ha hashtags
    pshBscore=HsStanceIndication(ht,tweets,nodes,color,1)    
    ht=pshAscore.keys()+pshBscore.keys() #new hashtag set by combining Ha and Hb
    #fetch tweets based on hashtags passed(#currently tweets that are authored by community member is only included;
    #later all the tweets even if not authored by members will be include)
    tweets=fetchtweetsHs(ht,tweets) #tweets structure (txt,ht list)
    #here onwards only these tweets will be used for calculations of score because pshscore is available only for them        
    #twthtprob=tweet2vec(categorydir,ht,tweets)#tweet hashtag probability
    # for Ha, tweetstanceindication score calculation(equatino 2 of paper)
    scoreA=list()    
    for i,tup in enumerate(tweets):
        tot=0        
        # select the tweets for stance A only when one of the Ha hashtag is common with the tweet(otherwise score will be zero and tweet will be not selected automatically)
        if(len(set(tup[1]).intersection(set(pshAscore.keys()))) >0):
            #print(tup[1],tup)
            for j,hs in enumerate(pshAscore.keys()):                        
                tot+=pshAscore[hs]*1#twthtprob[i][j]
            scoreA.append([i,tot]) # index i for tracking the tweet in tweets list
    scoreB=list()    
    #print("b starts here")
    for i,tup in enumerate(tweets):
        tot=0
        if(len(set(tup[1]).intersection(set(pshBscore.keys()))) >0):
            #print(tup[1],tup)
            for j,hs in enumerate(pshBscore.keys()):            
                k=len(pshAscore.keys())+j #to give the real index in twthtprob
                tot+=pshBscore[hs]*1#twthtprob[i][k]
            scoreB.append([i,tot])  # index i for tracking the tweet in tweets list  
    return tweets,scoreA,scoreB,pshAscore.keys(),pshBscore.keys()
...
def GraphGeneration(categorydir):
    ...
    plt.clf()
    return nodes
def GraphGeneration1(categorydir):
    adjdict={} #act as ajacency list with weights node:(adjnode,weights),(adjnode1,weights)
    tweetlst={} # each node(user) and its corresponding tweets
    tempdir=categorydir
    categorydir=categorydir+"/data"
...
def HsStanceIndication(ht,tweets,nodes,color,com):   
    ...
    if(HsStanceIndication.state==0):
        nhtig=TopNInformationGain(ht,min(500,len(ht)),nodes,color)  #common for community A and B      
...
def callclassifier(trainx,ytrain,filename):
...
def classify():    
    ...
    read_corpus("data/tempebola.txt","testgensim.txt", tokens_only=True)  
    with open("testgensim.txt","r") as f:
        tweets = [x for x in f.readlines()]
    #print tweets
    x_train, x_vald, y_train, y_vald = train_test_split(tweets, ytrain, test_size = 0.1,stratify=ytrain)    
    x_train=pd.DataFrame(x_train,dtype='string',columns=['sentence'])
    x_vald=pd.DataFrame(x_vald,dtype='string',columns=['sentence'])
    y_train=pd.DataFrame(y_train,dtype='int')
    y_vald=pd.DataFrame(y_vald,dtype='int')
    print(x_train.dtypes,y_train.dtypes)

    import tensorflow as tf
    import tensorflow_hub as hub
    module_url = "./USE/3"#"https://tfhub.dev/google/universal-sentence-encoder-large/3?tf-hub-format=compressed"
    sttime=time.time()
    #make in temporary directory
    ...
    for j,message_embedding1 in enumerate(message_embeddings):
        t.append([])
        for i,message_embedding2 in enumerate(message_embeddings):
            t[j].append([i,np.inner(message_embedding1, message_embedding2)/(norm(message_embedding1)*norm(message_embedding2))])    
    
    sortindex = [sorted(y,key=lambda x:x[1], reverse=True)[0:30] for y in t]
    #print sortindex
    for i,x in enumerate(sortindex):
        print(tweets[i]+":\n")
        count=0
        for y in x:
            if y[1]>0.7:
                ...
                break            
        if(count>15):            
            print('', '\n'.join([tweets[z[0]] for z in x[0:count+1]]))
        print("\n")    
    
    '''
    # Training input on the whole training set with no limit on training epochs.
    train_input_fn = tf.estimator.inputs.pandas_input_fn(x_train, y_train, num_epochs=None, shuffle=True)
    # Prediction on the whole training set.
    predict_train_input_fn = tf.estimator.inputs.pandas_input_fn(x_train, y_train, shuffle=False)
    # Prediction on the test set.
    predict_test_input_fn = tf.estimator.inputs.pandas_input_fn(x_vald, y_vald, shuffle=False)
    embedded_text_feature_column = hub.text_embedding_column(key="sentence", module_spec=module_url,trainable=True)
    estimator = tf.estimator.DNNClassifier(hidden_units=[512, 128],feature_columns=[embedded_text_feature_column],
    n_classes=7,optimizer=tf.train.AdagradOptimizer(learning_rate=0.005))
    ...
    #x_train, x_vald, y_train, y_vald = train_test_split(message_embeddings, ytrain, test_size = 0.1)#,stratify=ytrain)    
    model1=callclassifier(message_embeddings,ytrain,"data/tempebola.txt")    
    '''
    '''
    #print x_train
    #using universal sentence encoder
    train_input_fn = tf.estimator.inputs.numpy_input_fn(
    {'sentence': x_train}, y_train,batch_size=50, num_epochs=None, shuffle=True)    
    # Prediction on the whole training set.
    predict_train_input_fn = tf.estimator.inputs.numpy_input_fn(
        {'sentence': x_train}, y_train, shuffle=False)        
    # Prediction on the whole validation set.
    predict_val_input_fn = tf.estimator.inputs.numpy_input_fn(
    {'sentence': x_vald}, y_vald, shuffle=False)
    
    embedding_feature = hub.text_embedding_column(key='sentence', module_spec=module_url,trainable=False)
    ...
    val_eval_result = dnn.evaluate(input_fn=predict_val_input_fn)
    ...
    '''print(
      ...
      "Training Accuracy": val_eval_result["accuracy"],
      "Training AUC": val_eval_result["auc"],
      "Training Precision": val_eval_result["precision"],
      "Training Recall": val_eval_result["recall"]            )    
    tf.logging.set_verbosity(tf.logging.ERROR)
    results = {}
